...
void WriteReadToRef(const std::string &path, const std::unordered_map<uint32_t, ec_info> &ecs) {
  File::Out file(path + "_read-to-ref.txt");
  if (file.stream().good()) {
    for (auto ec : ecs) {
      file.stream() << ec.first << ' ';
      for (size_t i = 0; i < ec.second.pseudoalignment.size(); ++i) {
	if (ec.second.pseudoalignment.at(i)) {
	  file.stream() << i << (i == ec.second.last_val ? '\n' : ' ');
	}
      }
    }
    file.close();
  } else {
    throw std::runtime_error("File " + path + "_read-to-ref.txt" + " is not writable.");
  }
}
...
int main(int argc, char* argv[]) {
  cxxargs::Arguments args("telescope-intersection", "Usage:...");
  try {
  ...
  ReadIntersectionAlignments(args.value<std::string>('1'), ecs, args.value<uint32_t>("n-refs"));
  ReadIntersectionAlignments(args.value<std::string>('2'), ecs, args.value<uint32_t>("n-refs"));
  WriteReadToRef(args.value<std::string>('o'), ecs);
  const std::unordered_map<std::vector<bool>, ec_info> &cmp_ecs = CompressAlignments(ecs);
  WriteFullAlignments(args.value<std::string>('o'), cmp_ecs);
