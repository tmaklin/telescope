...
int main(int argc, char* argv[]) {
  ...
  //ReadFullAlignments(infile_1, ecs);
  //ReadFullAlignments(infile_2, ecs);
  ReadUnionAlignments(infile_1, ecs, n_refs);
  ReadUnionAlignments(infile_2, ecs, n_refs);
  WriteReadToRef(outfile, ecs);
  const std::unordered_map<std::vector<bool>, ec_info> &cmp_ecs = CompressAlignments(ecs);
  WriteFullAlignments(outfile, cmp_ecs);
